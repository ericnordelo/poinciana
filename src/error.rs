...
use crate::{
    hir::combiner,
    span,
    syntax::tree::{parser, semantics, tokenizer},
    utils::repeat_str,
};
...
impl<'e, E: fmt::Display> fmt::Display for Formatter<'e, E> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if start_offset == end_offset && start_offset == 0 {
            write!(f, "poinciana error: {}", self.err)?;
            return Ok(());
        }

        writeln!(f, "poinciana error: {}\n", self.err)?;
...
mod test {
    ...
    fn test_notate() {
        ...
        expected.push('\n');
        expected.push_str(format!("poinciana error: {}\n\n", formatter.err).as_str());
...
└── when 2"
        ...
        let expected = r"•••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••••
poinciana error: found a condition with no children
